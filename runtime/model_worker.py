...
def identity(a):
    ...
    return hashlib.sha256(encoded(dict(files=files,rms=float(a.rms),threads=os.getenv('OMP_NUM_THREADS','4'),
        beam=os.getenv('GEIST_WHISPER_BEAM_SIZE','5'),temperature_fallback=os.getenv('GEIST_WHISPER_TEMPERATURE_FALLBACK','1'),chunk_seconds=os.getenv('GEIST_WHISPER_CHUNK_SECONDS','28'),audio_context=os.getenv('GEIST_WHISPER_AUDIO_CONTEXT','full'),wait=os.getenv('OMP_WAIT_POLICY',''),trace=os.getenv('GEIST_DIKTAT_TRACE','')))).hexdigest()
