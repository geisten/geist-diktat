...
Im Grilling-Interview bestätigt: Geist-Diktat wird als erste öffentliche Beta für Ubuntu GNOME geplant, mit direktem Diktat und dokumentierter lokaler Prozessschnittstelle. Die beste geprüfte Engine darf Geist/Gemma ersetzen; Pi5 und macOS blockieren diese Beta nicht. Die eigene Stärke liegt in Aufnahme, zuverlässiger Sitzungssteuerung und Einbettung in Anwendungen. Das ASR-Modell wird anhand deutscher Qualität, Einfügelatenz, Speicherbedarf und Betriebsstabilität ausgewählt. Ein einzelner Leaderboard-Platz beantwortet diese Produktfrage nicht.

**Erster konkreter Versuch: residenter whisper.cpp-Adapter für das Ubuntu-Desktopprofil.** Pi5 small Q5_1 / Beam 1 bleibt ein separates experimentelles Folgeprofil. Der eigene Vergleich belegt bereits erheblich weniger RAM und weniger Wortfehler als Geist/Gemma. Er belegt noch keine kontinuierliche Nutzbarkeit: Beide bisher geprüften Adapter überlasten auf dem Pi5 bei zeitgetreuer Gesprächszufuhr. Daher wird der Standardbackend noch nicht allein aufgrund der zwölf Pilotaufnahmen umgestellt.
...
Bestätigte Ausgabepolitik: Ein Shortcut startet/stoppt, stabile Abschnitte werden nach Sprechpausen eingefügt. Für die technische Stabilisierung der Ergebnisse gibt es verschiedene Verfahren: WhisperStreaming untersucht wiederholt übereinstimmende Präfixe, während dessen Autoren inzwischen SimulStreaming als Nachfolger empfehlen. SimulStreaming ist eine nützliche Forschungsreferenz für die Entscheidung, wann Text endgültig ausgegeben wird; die dokumentierte leistungsstarke Konfiguration setzt GPU-Ressourcen voraus. Ein Übertragen auf den Pi5 ist eigene Entwicklungsarbeit mit offener Laufzeit. [WhisperStreaming](https://github.com/ufal/whisper_streaming), [SimulStreaming](https://github.com/ufal/SimulStreaming).

Die erste Umsetzung sollte deshalb keinen umfassenden Framework-Wechsel erzwingen: den vorhandenen Prozessvertrag beibehalten, im eigenen Adapter einen langlebigen Kontext nutzen und Fenster/Endpunkte kontrolliert variieren. Wenn der Ubuntu-Pfad die Abnahmetore verfehlt, folgt ein gezielter weiterer Modell-/Runtime-Vergleich. Eine vollständige Modellrangliste ist kein Beta-Tor. Für das spätere Pi-Profil können ein kleineres oder nachweislich deutschfähiges natives Streamingmodell nötig werden. Kein ungeprüftes englisches Streamingmodell erfüllt den deutschen Produktauftrag.
...
Ein VAD löst weder Dialektorthographie noch Halluzinationen während echter Sprache. Die erste Beta hat genau einen Ausgabeauftrag: inhaltstreues Standarddeutsch ohne stilistische Umschreibung. Sie benötigt dafür menschliche Referenzen und regionale Beurteilung. Die historische Messung gegen Dialektschreibung bleibt eine getrennte Forschungsaufgabe; daraus entsteht kein zweiter Beta-Modus. DACH wird breit getestet und zunächst experimentell ausgewiesen. Für DACH werden deshalb zusätzliche eigene/rechtegeklärte Aufnahmen eingeplant. SwissDial- und Gesprächspilot bleiben Forschungsevidenz; eingeschränkte Audios und vollständige Referenzen werden nicht ungeprüft in öffentliche CI-Artefakte übernommen.
...
Für die erste öffentliche Beta gilt verbindlich der [Pilotvergleich gegen Tippen](BETA-PILOT.md): mindestens fünf externe Personen, getrennte Alltags- und technische Texte, jeweils mindestens 25 % Zeitgewinn im Median und mindestens vier von fünf Personen schneller, einschließlich vollständiger Korrektur. Germar gewinnt die Tester; die Kriterien bestimmen den Termin. Öffentliche Vorstellung und installierbarer Download erfolgen gemeinsam nach bestandenem Pilot.

Für einen späteren zusätzlichen Vergleich mit etablierten Diktatprodukten dieselben Nutzer vergleichbare Diktataufgaben in wechselnder Reihenfolge erledigen lassen: Zielsystem versus etablierter lokaler Referenzclient bzw. Betriebssystem-Diktat, sofern auf der Plattform verfügbar. Erfassen: erfolgreiche Einfügung, Korrekturzeit pro 100 Wörter, Fehlaktivierungen, Startzeit, Einfügelatenz und Einrichtungsaufwand. Reine WER und reine Code-Coverage reichen dafür nicht. Ein SOTA-Anspruch bleibt solange eine Entwicklungsrichtung, bis auf diesen Zielaufgaben ein belastbarer Vorteil belegt ist.
